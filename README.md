...
The `save` and `load` functions provided to `createAuthClient` allow you to
persist tokens between application restarts:
...
- The `refreshTokens()` method will throw a `SchwabAuthError` with code
  `TOKEN_EXPIRED`
...
See [BETA-RELEASE.md](./BETA-RELEASE.md) for instructions on publishing beta
