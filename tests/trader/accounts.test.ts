...
import { getAccounts, getAccountByNumber } from '../../src/trader/accounts/endpoints';
...
describe('Trader API - Accounts Endpoints', () => {
  beforeEach(() => {
    ...
    configureSchwabApi({ baseUrl: 'https://api.test.schwabapi.com', enableLogging: false });
  });
  ...
  describe('getAccounts', () => {
    ...
    it('should retrieve accounts with fields query parameter', async () => {
        const mockAccountsResponse = [
          { securitiesAccount: { accountId: '123', hashedAccountId: 'abc', positions: [] } },
        ];
        mockFetch.mockResolvedValueOnce({
          ok: true,
          json: async () => mockAccountsResponse,
  ...
  describe('getAccountByNumber', () => {
    it('should retrieve a specific account successfully', async () => {
      const mockAccountResponse = { securitiesAccount: { accountId: '123', hashedAccountId: 'abc' } };
      mockFetch.mockResolvedValueOnce({
        ok: true,
      ...
      const account = await getAccountByNumber('test-token', { pathParams: { accountNumber: 'abc' } });
      expect(mockFetch).toHaveBeenCalledWith(
    ...
    it('should throw SchwabApiError if account not found (404)', async () => {
        ...
        await expect(getAccountByNumber('test-token', { pathParams: { accountNumber: 'nonexistent' } }))
